...
class Suction(Gripper):
  ...
  def activate(self):
    ...
    if not self.activated:
      points = p.getContactPoints(bodyA=self.parent_robot, linkIndexA=self.ee_link_tip)
      # print(points)
      if points:
        ...
        if obj_id in self.obj_ids['rigid']:
          body_pose = p.getLinkState(self.parent_robot, self.ee_link_tip)
          ...
          self.contact_constraint = p.createConstraint(
              parentBodyUniqueId=self.parent_robot,
              parentLinkIndex=self.ee_link_tip,
  ...
  def detect_contact(self):
    ...
    body, link = self.parent_robot, self.ee_link_tip
    ...
    # exit()
    if self.activated:
      #   TODO how to detect contact by this check!!!(zhangkefang)
      points = [point for point in points if point[2] != self.parent_robot]
    # else:
    #   points = [point for point in points if not (point[2] == self.parent_robot
    #                                           and point[3] == self.ee_link_tip
    #                                           and point[4] == self.ee_link_tip + 1)]
