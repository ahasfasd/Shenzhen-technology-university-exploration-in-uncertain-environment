...
EPSILON = 0.9
OBSTACLE_NUM = 1
ENV_PARAMS = {'obs': 7, 'goal': 1, 'action': 6}



ENV_PARAMS['obs'] = (OBSTACLE_NUM+8)*10+3
# ##############################  DDPG  ####################################


class ANet(nn.Module):   # ae(s)=a
    def __init__(self, init_w = 3e-2):
        super(ANet, self).__init__()
        self.fc1 = nn.Linear(ENV_PARAMS['obs'] + ENV_PARAMS['goal'], 512)
        self.fc2 = nn.Linear(512, 1024)
        self.fc3 = nn.Linear(1024, 1024)
        self.fc4 = nn.Linear(1024, ENV_PARAMS['action'])
...
class CNet(nn.Module):
    def __init__(self, init_w = 3e-2):
        ...
        self.fc1 = nn.Linear(ENV_PARAMS['obs'] + ENV_PARAMS['action']+ENV_PARAMS['goal'], 512)
        self.fc2 = nn.Linear(512, 1024)
        self.fc3 = nn.Linear(1024, 1024)
        self.fc4 = nn.Linear(1024, ENV_PARAMS['action'])
...
class DDPG(object):
    ...
    def save_mode(self):
        torch.save(self.Actor_eval, "model/Actor_eval.pkl")
        torch.save(self.Actor_target, "model/Actor_target.pkl")
        torch.save(self.Critic_eval, "model/Critic_eval.pkl")
        torch.save(self.Critic_target, "model/Critic_target.pkl")

    def load_mode(self):
        self.Actor_eval = torch.load("model/Actor_eval.pkl")
        self.Actor_target = torch.load("model/Actor_target.pkl")
        self.Critic_eval = torch.load("model/Critic_eval.pkl")
        self.Critic_target = torch.load("model/Critic_target.pkl")